...
public abstract class Stream extends Operator
{
    ...
    // Yousun Ko: name of the file to record constant variables
    private static String recordVarFile = "const_vars.out";

    // Yousun Ko: define recordVar
    public void recordVar(String msg){
        String filename = toplevel.toString()+"."+recordVarFile;
        PrintWriter out = null;
        try { 
            out = new PrintWriter(new FileOutputStream(new File(filename), true)); // append = true
            out.println(msg);
            out.close();
        }catch (Throwable e){
            ERROR(e);
    ...
    private static String recordPathFile = "filename.out";
    // Yousun Ko: record input file name
    public void recordFileNameIn(String path) {
        String filename = toplevel.toString()+"."+recordPathFile;
        PrintWriter out = null;
        try { 
            out = new PrintWriter(new FileOutputStream(new File(filename), true)); // append = true
            out.println("InputFile,"+path);
            out.close();
        }catch (Throwable e){
            ERROR(e);
        }
    ...
    // Yousun Ko: record output file name
    public void recordFileNameOut(String path) {
         String filename = toplevel.toString()+"."+recordPathFile;
        PrintWriter out = null;
        try { 
            out = new PrintWriter(new FileOutputStream(new File(filename), true)); // append = true
            out.println("OutputFile,"+path);
            out.close();
        }catch (Throwable e){
            ERROR(e);
        }
    ...
    // Yousun Ko: return incoming data type of the stream
    public String getIncomingType(){
        if (inputChannel != null){
            return inputChannel.getType().toString();
        }else{
            return null;
        }
    }

    // Yousun Ko: return outgoing data type of the stream
    public String getOutgoingType(){
        if (outputChannel != null){
            return outputChannel.getType().toString();
        }else{
            return null;
        }
    }
    ...
    */
    ...
    public void run(String args[])
    {
        ...
        // Yousun Ko: generate file
        String filename = toplevel.toString()+"."+recordVarFile;
        PrintWriter out = null;
        try { 
            out = new PrintWriter(new FileOutputStream(new File(filename), false)); // append = false
            out.close();
        }catch (Throwable e){
            ERROR(e);
        }
