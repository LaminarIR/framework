...
public class MoveStreamParameters extends InitMunger
{
    ...
    // Yousun Ko: pass specName as an argument for convenience
    private Function addInitParams(String specName, Function init, List params)
    {
        ...
        for (Iterator iter = params.iterator(); iter.hasNext(); )
        {
            ...
            Parameter newParam = new Parameter(type, pName);
            while (type instanceof TypeArray){
                type = ((TypeArray)type).getBase();
            }
            if (type instanceof TypeStruct){
                rhs = new ExprTypeCast(context, newParam.getType(), rhs);
                newParam = new Parameter(objectType, pName);
            }
            Statement stmt = new StmtAssign(context, lhs, rhs);
            body.add(stmt);

            // Yousun Ko: begin
            // add `recordVar' function call expression which prints 
            // parameters on console. Node `recordVar' will be replaced by
            // `System.out.println' during code generation.
            // possible extension: switchable recordVar replacement 
            //                     e.g., file I/O
            List recordParams = new ArrayList();
            String param_val = "";
            // use deepToString/toString to print array elements
            if (param.getType() instanceof TypeArray){
                if (((TypeArray)param.getType()).getDims() >= 2){
                    param_val = "java.util.Arrays.deepToString("+param.getName()+")"; 
                } else {
                    param_val = "java.util.Arrays.toString("+param.getName()+")"; 
                }
            }else{
                param_val = param.getName();
            }
            recordParams.add(new ExprConstStr(context, "\""+specName+"_\"+this.hashCode()+\","+param.getName()+",\"+"+param_val));

            ExprFunCall recordExpr = new ExprFunCall(context, "recordVar", recordParams);
            Statement recordStmt = new StmtExpr(context, recordExpr);
            body.add(recordStmt);
            // Yousun Ko: end
    ...
    private Function addInitParamsOnly(String specName, Function init, List params)
    {
        FEContext context = init.getContext();
        ...
        for (Iterator iter = params.iterator(); iter.hasNext(); )
        {
            ...
            // Yousun Ko: begin
            // add `recordVar' function call expression which prints 
            // parameters on console. See `addInitParams' method for more
            // information.
            List recordParams = new ArrayList();
            String param_val = "";
            if (param.getType() instanceof TypeArray){
                ...
                if (spec.getType() == StreamSpec.STREAM_FILTER)
                    {
                        ...
                        // Yousun Ko: function def is revised.
                        init = addInitParams(spec.getName(), init, spec.getParams());
                        newFuncs.add(init);
                ...
                else
                    {
                        ...
                        newFuncs.remove(init);
                        // Yousun Ko: function def is revised.
                        init = addInitParamsOnly(spec.getName(), init, spec.getParams());
                        newFuncs.add(init);
                    }
