...
public class NodesToJava implements FEVisitor
{
    ...
    public Object visitExprFunCall(ExprFunCall exp)
    {
        ...
        } else if (name.equals("recordVar")) {
            // Yousun Ko: escape `recordVar' function from being a 
            // `Math' member.
            result = name + "("; 
        } else {
    ...
    public Object visitStmtAdd(StmtAdd stmt)
    {
        ...
        result = (String) doStreamCreator("add", stmt.getCreator());

        // Yousun Ko: Invoke recordFileName function to store paths to files 
        SCSimple sc = (SCSimple)stmt.getCreator();
        String scName = sc.getName();
        if (scName.equals("FileReader") || 
            scName.equals("FileWriter")){
            String recordFileName = "";

            if (scName.equals("FileReader")){
                recordFileName = "recordFileNameIn(";
            }else if(scName.equals("FileWriter")){
                recordFileName = "recordFileNameOut(";
            }
            
            if (sc.getParams().get(0) instanceof ExprConstStr) {
                recordFileName += (String)sc.getParams().get(0).accept(this);
            }else{
                System.err.println("Error: expected first argument to " + scName
                    + " to be a String (the filename)");
                System.exit(1);
            }
            recordFileName += ")";
            result += ";\n" + indent + recordFileName;
        }
        return result;
